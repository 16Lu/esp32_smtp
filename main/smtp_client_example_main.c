...
//在 mbedtls 库函数返回错误码时，直接跳转到指定的错误处理逻辑
...
static int write_and_get_response(mbedtls_net_context *sock_fd, unsigned char *buf, size_t len)
{
    ...
    do {
        ...
        printf("sock_fd: %d\n", sock_fd->fd);
        printf("sock_fd: %p, buf: %p, len: %zu\n", (void*)sock_fd, (void*)buf, len);
        printf("buf: ");
...
static void smtp_client_task(void *pvParameters)
{
    ...
    len = snprintf((char *) buf, BUF_SIZE,
                   ...
                   "ESP32 SMTP Client <liangye0322@163.com>", RECIPIENT_MAIL);
